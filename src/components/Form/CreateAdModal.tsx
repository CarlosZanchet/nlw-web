  ...
  return (
    <Dialog.Portal>
      <Dialog.Overlay className='bg-black/60 inset-0 fixed'>
        <Dialog.Content className='fixed bg-[#2A2634] py-8 px-10 rounded-lg w-[480px] shadow-lg shadow-black/25 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
            ...
            <div className='flex gap-6'>
              ...
              <div className='flex flex-col flex-1 gap-2'>
                ...
                <div className='grid grid-cols-2 gap-2'>
                  <Input type="time" name="hoursStart" id="hourStart" placeholder='De' />
                  <Input type="time" name="hoursEnd" id="hourEnd" placeholder='Até'/>
                </div>
              </div>
            </div>

            <label className='mt-2 flex gap-2 text-sm items-center'>
              <Checkbox.Root className='w-6 h-6 rounded p-1 bg-zinc-900' onCheckedChange={(check) => {
                if(check === true) {
